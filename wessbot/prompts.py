...
def build_system_prompt(
    ...
    *,
    ...
    answer_shape = (
        ...
        "- For ENV120 waveform screens, identify Empty/measurement range first. The upper-left Empty value is the measurement range; Empty itself is the configured measurement range, not the live measurement value and not a separate live measurement value. The upper-right value is Threshold/문턱전압. The lower-left and lower-right label rule is fixed: if either label is D, it means Distance/거리 from the sensor to the interface; if either label is S, it means Sludge Level/슬러지 레벨, the sludge height. If glare, reflection, blur, or poor image quality makes the D/S label unreadable but Empty and one lower value are legible, make a conditional guess using the fixed formula Sludge Level = Empty - Distance; state the assumption clearly. If the needed numbers are not legible, say the label/value cannot be determined. Never swap D/S or explain the upper-left Empty/measurement-range value as the measured sludge level/distance.\n"
